...
public class StickerView extends FrameLayout {
    ...
    public StickerView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        try {
            ...
            borderPaint.setColor(a.getColor(R.styleable.StickerView_sborderColor, Color.BLACK));
...
//            configDefaultIcons();
  ...
  /*  public void configDefaultIcons() {
        BitmapStickerIcon deleteIcon = new BitmapStickerIcon(
                ContextCompat.getDrawable(getContext(), R.drawable.ic_cancel_small),
        ...
        BitmapStickerIcon flipIcon = new BitmapStickerIcon(
                ContextCompat.getDrawable(getContext(), R.drawable.ic_rotation),
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_POINTER_DOWN:
                ...
                if (handlingSticker != null && event.getPointerCount() > 1 && isInStickerArea(handlingSticker, event.getX(1),
...
//                    midPoint.y);
    ...
    public Boolean removeStickerlist(Sticker sticker) {
    ...
    public boolean remove(@Nullable Sticker sticker) {
        if (stickers.contains(sticker)) {
            stickers.remove(sticker);
            Log.e(TAG, "remove() returned: " + stickers.size());
            //remove shubhangi
//            if (onStickerOperationListener != null) {
...
//            }
    ...
    @NonNull
    public StickerView addSticker(@NonNull Sticker sticker, float scale) {
        return addSticker(sticker, Sticker.Position.CENTER, scale);
    }
    ...
    public StickerView addSticker(@NonNull final Sticker sticker,
                                  final @Sticker.Position int position, final float scale) {
    ...
    protected void addStickerImmediately(@NonNull Sticker sticker, @Sticker.Position int position, float scale) {
        ...
        sticker.getMatrix().postScale(scaleFactor / scale, scaleFactor / scale, getWidth() / 2, getHeight() / 2);
    ...
    public void addCopySticker(@NonNull Sticker sticker, Boolean drawableStricker) {
        handlingSticker = sticker;
        if (drawableStricker) {
            stickers.add(sticker);
            if (onStickerOperationListener != null) {
    ...
    public void centerCropImage(Bitmap bitmap, int width, int height) {

        int picWidth = bitmap.getWidth();
        int picHeight = bitmap.getHeight();

        Matrix matrix = new Matrix();
        float maxScale = Math.max((float) width / picWidth, (float) height
                / picHeight);
        matrix.postScale(maxScale, maxScale);
        if ((float) width / picWidth > (float) height / picHeight)
            matrix.postTranslate(0, (height - picHeight * maxScale) / 2);
        else
            matrix.postTranslate((width - picWidth * maxScale) / 2, 0);

        MIN_ZOOM = Math.min((float) width / picWidth, (float) height
                / picHeight);
    ...
    public void setCurrentSelected(Sticker sticker) {
        handlingSticker = sticker;
        invalidate();
    }

   public void changeMatrix(Sticker sticker, Matrix matrix) {
