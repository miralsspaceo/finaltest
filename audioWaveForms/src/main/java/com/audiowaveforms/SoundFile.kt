...
//                    .log("Could not load waveform of extention $fileName")
                return null
//                throw Exception("Not supported file extension. $fileName")
            }
...
//                throw FileNotFoundException(fileName)
    ...
    private fun readFile(inputFile: File) {
        Log.e("SoundFile", "readFile start")
        ...
        var value: Int
        val frameLens =
            (1000 * avgBitrateKbps / 8 * (samplesPerFrame.toFloat() / sampleRate)).toInt()
        i = 0
        ...
        while (i < numFrames) {
            ...
            mFrameOffsets!![i] =
                (i.toFloat() * (1000 * avgBitrateKbps / 8).toFloat() * (samplesPerFrame.toFloat() / sampleRate)).toInt()
            i++
        }

        Log.e("SoundFile", "readFile end")
